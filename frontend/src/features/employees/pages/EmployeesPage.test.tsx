...
vi.mock('../components/EmployeeList', () => ({
  EmployeeList: ({ onCreateEmployee, onEditEmployee, onViewEmployee, onImportEmployees, onExportEmployees }: any) => (
    <div data-testid="employee-list">
      <button onClick={onCreateEmployee}>Create Employee</button>
      <button onClick={() => onEditEmployee({ id: 1, firstName: 'John', lastName: 'Doe' })}>Edit Employee</button>
      <button onClick={() => onViewEmployee({ id: 1, firstName: 'John', lastName: 'Doe' })}>View Employee</button>
...
vi.mock('../components/EmployeeImport', () => ({
  EmployeeImport: ({ opened, onClose, onSuccess }: any) => (
    <div data-testid="employee-import" style={{ display: opened ? 'block' : 'none' }}>
      <button onClick={onClose}>Close Import</button>
      <button onClick={() => onSuccess([{ id: 1, firstName: 'John', lastName: 'Doe' }])}>Success Import</button>
    </div>
  ),
}));
...
vi.mock('../components/EmployeeExport', () => ({
  EmployeeExport: ({ opened, onClose, selectedEmployees }: any) => (
    <div data-testid="employee-export" style={{ display: opened ? 'block' : 'none' }}>
